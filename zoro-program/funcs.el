;;; funcs.el --- zorow Layer packages File for Spacemacs
;;
...
(defun zorowk/js2-which-function ()
...
(defun zorowk-refresh-imenu-index ()
  (interactive)
...
(defun zorowk/merge-company-tabnine-to-company-lsp ()
  (when (memq 'company-lsp company-backends)
    (setq-local company-backends (remove 'company-lsp company-backends))
    (add-to-list 'company-backends '(company-lsp :with company-tabnine :separate)))
  )

(defun zorowk/company-box-icons--tabnine (candidate)
  (when (eq (get-text-property 0 'company-backend candidate)
            'company-tabnine)
    'Reference))

(defun zorowk/sort-by-tabnine (candidates)
