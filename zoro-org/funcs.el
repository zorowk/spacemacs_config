;;; funcs.el --- zorowk Layer packages File for Spacemacs
;;
...
;; Screenshot
(defun zorowk//insert-org-or-md-img-link (prefix imagename)
  (if (equal (file-name-extension (buffer-file-name)) "org")
      (insert (format "[[%s%s]]" prefix imagename))
    (insert (format "![%s](%s%s)" imagename prefix imagename))))

(defun zorowk/capture-screenshot (basename)
  ...
  (if (file-exists-p (file-name-directory fullpath))
      (progn
        ...
        (zorowk//insert-org-or-md-img-link "../img/" relativepath))
    (progn
      (call-process "screencapture" nil nil nil "-s" (concat basename ".png"))
      (zorowk//insert-org-or-md-img-link "./" (concat basename ".png"))))
  (insert "\n"))

(defun zorowk/org-archive-done-tasks ()
  (interactive)
  (org-map-entries
   (lambda ()
     (org-archive-subtree)
...
(defun zorowk/org-archive-cancel-tasks ()
  (interactive)
  (org-map-entries
   (lambda ()
     (org-archive-subtree)
...
(defun zorowk/skip-non-stuck-projects ()
...
(defun zorowk/org-insert-src-block (src-code-type)
...
(defun zorowk/org-summary-todo (n-done n-not-done)
  "Switch entry to DONE when all subentries are done, to TODO otherwise."
  (let (org-log-done org-log-states)    ; turn off logging
    (org-todo (if (= n-not-done 0) "DONE" "TODO"))))

(defun zorowk/filter-by-tags ()
  (let ((head-tags (org-get-tags-at)))
    (member current-tag head-tags)))

(defun zorowk/org-clock-sum-today-by-tags (timerange &optional tstart tend noinsert)
  (interactive "P")
